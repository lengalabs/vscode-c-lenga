...
export function NodeRender({ node, indent }: NodeRenderProps): React.ReactNode {
  switch (node.type) {
    ...
    case "FunctionDeclaration":
      return (
        <FunctionDeclarationRender
          functionDeclaration={node as nodes.FunctionDeclaration}
        />
      );
    case "FunctionDefinition":
      return (
        <FuncDefRender
          funcDef={node as nodes.FunctionDefinition}
          indent={indent}
        />
      );
    case "Declaration":
    ...
    case "CompoundStatement":
      return (
        <CompStmtRender
          compStmt={node as nodes.CompoundStatement}
          indent={indent}
        />
      );
    case "IfStatement":
      return (
        <IfStatementRender
          ifStatement={node as nodes.IfStatement}
          indent={indent}
        />
      );
    case "ElseClause":
      return (
        <ElseClauseRender
          elseClause={node as nodes.ElseClause}
          indent={indent}
        />
      );
    case "CallExpression":
    ...
    case "AssignmentExpression":
      return (
        <AssignmentExprRender
          assignmentExpr={node as nodes.AssignmentExpression}
        />
      );
    case "NumberLiteral":
      return (
        <NumberLiteralExprRender literalExpr={node as nodes.NumberLiteral} />
      );
    case "StringLiteral":
      return (
        <StringLiteralExprRender literalExpr={node as nodes.StringLiteral} />
      );
    case "BinaryExpression":
      return (
        <BinaryExpressionRender
          binaryExpression={node as nodes.BinaryExpression}
        />
      );
    default:
      return "WIP";
...
function IncludeDeclRender({
  includeDecl,
}: {
  includeDecl: nodes.PreprocInclude;
}): React.ReactNode {
  return (
    <>
...
function FunctionDeclarationRender({
  ...
  functionDeclaration: nodes.FunctionDeclaration;
}): React.ReactNode {
...
function FuncDefRender({
  funcDef,
  indent,
}: {
  funcDef: nodes.FunctionDefinition;
  indent: number;
}): React.ReactNode {
  const { nodeMap } = useLineContext();
  ...
  return (
    <>
      ...
      <span className="token-delimiter">{")"}</span>
      {funcDef.body && (
        <CompStmtRender compStmt={funcDef.body} indent={indent} />
      )}
    </>
  );
}

function VarDeclRender({
  varDecl,
}: {
  varDecl: nodes.Declaration;
}): React.ReactNode {
  const { nodeMap } = useLineContext();
...
function ParamDeclRender({
  paramDecl,
}: {
  paramDecl: nodes.FunctionParameter;
}): React.ReactNode {
  const { nodeMap } = useLineContext();
...
function CompStmtRender({
  compStmt,
  indent,
}: {
  compStmt: nodes.CompoundStatement;
  indent: number;
}): React.ReactNode {
  return (
    <>
...
function IfStatementRender({
  ...
  return (
    <>
      ...
      {
        <CompStmtRender
          compStmt={ifStatement.compoundStatement}
          indent={indent}
        />
      }
      {ifStatement.elseClause && (
        <ElseClauseRender elseClause={ifStatement.elseClause} indent={indent} />
      )}
    </>
  );
}
...
function ElseClauseRender({
  ...
  return (
    <>
      ...
      {
        <CompStmtRender
          compStmt={elseClause.compoundStatement}
          indent={indent}
        />
      }
    </>
...
function ReturnStmtRender({
  returnStmt,
}: {
  returnStmt: nodes.ReturnStatement;
}): React.ReactNode {
  return (
    <>
...
function CallExprRender({
  callExpr,
}: {
  callExpr: nodes.CallExpression;
}): React.ReactNode {
  return (
    <>
      <EditableField
...
function DeclRefExprRender({
  declRefExpr,
}: {
  declRefExpr: nodes.Reference;
}): React.ReactNode {
  const { nodeMap } = useLineContext();
...
function AssignmentExprRender({
  assignmentExpr,
}: {
  assignmentExpr: nodes.AssignmentExpression;
}): React.ReactNode {
  const { nodeMap } = useLineContext();
...
function NumberLiteralExprRender({
  literalExpr,
}: {
  literalExpr: nodes.NumberLiteral;
}): React.ReactNode {
  return (
...
function StringLiteralExprRender({
  literalExpr,
}: {
  literalExpr: nodes.StringLiteral;
}): React.ReactNode {
  return (
    <>
      <span className="token-string">{'"'}</span>
